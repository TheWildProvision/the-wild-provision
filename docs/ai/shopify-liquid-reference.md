# Shopify Liquid theme reference

General Shopify theme and Liquid reference for AI agents, from the Skeleton theme's original `AGENTS.md`. Load it when writing Liquid, schemas, blocks, sections or locale files.

> **Project overrides.** This file describes a _vanilla_ Shopify theme. Where it conflicts with the root [AGENTS.md](../../AGENTS.md), **AGENTS.md wins**. In particular:
>
> - There is no `assets/critical.css`. Global CSS is built by Vite from `src/styles/` into `assets/theme.css`.
> - Prefer Tailwind utility classes over `{% stylesheet %}`. When a `{% stylesheet %}` is genuinely needed, wrap its CSS in `@layer components { … }`.
> - JavaScript goes in `src/` (TypeScript), not `{% javascript %}` tags.
> - Colours, fonts and spacing come from design tokens, not theme settings. See [DESIGN_SYSTEM.md](../DESIGN_SYSTEM.md).
...
Variables that are declared with `decrement` are unique to the [layout](https://shopify.dev/docs/storefronts/themes/architecture/layouts), [template](https://shopify.dev/docs/storefronts/themes/architecture/templates),
or [section](https://shopify.dev/docs/storefronts/themes/architecture/sections) file that they're created in. However, the variable is shared across
[snippets](https://shopify.dev/docs/storefronts/themes/architecture/snippets) included in the file.
...
Variables that are declared with `increment` are unique to the [layout](https://shopify.dev/docs/storefronts/themes/architecture/layouts), [template](https://shopify.dev/docs/storefronts/themes/architecture/templates),
or [section](https://shopify.dev/docs/storefronts/themes/architecture/sections) file that they're created in. However, the variable is shared across
[snippets](https://shopify.dev/docs/storefronts/themes/architecture/snippets) included in the file.
...
directly accessible outside the snippet or app block. For example, a snippet or app block inside the [product template](https://shopify.dev/docs/storefronts/themes/architecture/templates/product)
can access the [`product` object](https://shopify.dev/docs/api/liquid/objects/product), and a snippet or app block inside a [section](https://shopify.dev/docs/storefronts/themes/architecture/sections)
...
- [`article_list` settings](https://shopify.dev/docs/storefronts/themes/architecture/settings/input-settings#article_list)
- [`collection_list` settings](https://shopify.dev/docs/storefronts/themes/architecture/settings/input-settings#collection_list)
- [`product_list` settings](https://shopify.dev/docs/storefronts/themes/architecture/settings/input-settings#product_list)
...
> [Pagination Limits](https://shopify.dev/docs/storefronts/themes/best-practices/performance/platform#pagination-limits) for more information.
...
To learn more about how JavaScript that's defined between the `javascript` tags is loaded and run, refer to the documentation for [javascript tags](https://shopify.dev/docs/storefronts/themes/best-practices/javascript-and-stylesheet-tags#javascript).
...
them in your theme, refer to [Render a section](https://shopify.dev/docs/storefronts/themes/architecture/sections#render-a-section).
...
Use this tag to render section groups as part of the theme's [layout](https://shopify.dev/docs/storefronts/themes/architecture/layouts) content. Place the `sections` tag where you want to render it in the layout.

To learn more about section groups and how to use them in your theme, refer to [Section groups](https://shopify.dev/docs/storefronts/themes/architecture/section-groups#usage).
...
> Note:
> If you reference [color settings](https://shopify.dev/docs/storefronts/themes/architecture/settings/input-settings#color) inside `style` tags, then
